...
void partidaBombaEstacionaria(unsigned char acao) { // a��o da bomba : 1 para ligar  / 2 : para desligar
    if (acao == 1 && !flagEstacionariaLigada) {
        if (_sec_partida != _sec) {
            ...
            if (SINAL_ESTAC_LIGADO) { // Verifica se a bomba a combustao ligou
                ...
                tempo_partida = 0; // zera tempo para caso nao ligar, come�e a contar novamente.
    ...
    if (tempo_limite_partida > tempo_limite_estacionaria) { // verifica se o tmepo tentando ligar � maior que o tempo limite ajustado.
    ...
    if (flagEstacionariaLigada) { // se a bomba estiver ligada
        if (SINAL_ESTAC_LIGADO) { // permanece verificando se a mesma nao desligou
            ...
            flagEstacionariaLigada = false; // Se a  bomba nao ligou, ou desligou no periodo, tentar ligar novamente, at� estourar o tempo maximo de partida
        }
    }
}

void verificarIntervaloTesteBombas() {
    if (dias_restante_teste >= intervalo_teste_dia && habilitaTesteAutomatico == 1 && !executandoTeste) { // Se ajustado intervalo como 0 entao desativa a fun�ao de teste automatico
...
void acaoTesteBombas() {
    if (btPress(b_esc)) {
        ...
        char i = 0;
        for (i = 0; i < 8; i++) {
            shift[i] = 0; // desliga todos os rel�s
        }
    }
